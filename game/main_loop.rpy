...
label week_start:
    ...
    if income_reduced_for > 0:
        $ income_reduced_for -= 1
    elif income_reduced_for == 0:
        $ income_reduced_for -= 1
        $ extra = castle.villages_income - (original_income / 1.2)
        $ castle.villages_income = original_income + extra

    if reduce_tavern_income == 2:
        $ castle.buildings["tavern"].income = 15
        $ reduce_tavern_income -= 1
    elif reduce_tavern_income > 0:
        $ reduce_tavern_income -= 1
    elif reduce_tavern_income == 0:
        $ castle.buildings["tavern"].income = 30
        $ reduce_tavern_income -= 1

    if original_research != 0:
        $ castle.tech = original_research
        $ original_research = 0

    if movement_points_penalised:
        $ movement_points_penalised = False
        $ avatar._mp = original_mp

    if reduced_orc_recruitment > 0:
        $ reduced_orc_recruitment -= 1
    elif reduced_orc_recruitment == 0:
        $ reduced_orc_recruitment -= 1
        $ castle.recruitment_bonuses["barracks"] += 1

    if iron_reduced_for > 0:
        $ iron_reduced_for -= 1
    elif iron_reduced_for == 0:
        $ reduced_orc_recruitment -= 1
