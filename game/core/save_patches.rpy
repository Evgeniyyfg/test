...
init -1 python:
    ...
    def sg_patch18():
        for uid in castle.buildings.keys():
            _dict_value = castle.buildings[uid].__dict__.copy()
            if 'bld_class' in all_buildings[uid]:
                castle.buildings[uid] = all_buildings[uid]['bld_class'](uid)
            else:
                castle.buildings[uid] = Building(uid)
            castle.buildings[uid].__dict__ = _dict_value

        for rs_cls in all_researches:
            if rs_cls.uid in castle.researches:
                _dict_value = castle.researches[rs_cls.uid].__dict__.copy()
            else:
                _dict_value = {}

            castle.researches[rs_cls.uid] = rs_cls()

            if _dict_value:
                castle.researches[rs_cls.uid].__dict__ = _dict_value
    ...
    def sg_patch20():
        for uid in castle.buildings:
            castle.buildings[uid].available = True

        castle.buildings["nasim_chamber"] = Building("nasim_chamber")

    def sg_patch21():
        m = create_map_from_file("maps/rosaria map.json")
        for index, i in enumerate(m.hexes):
            m.hexes[index][4] = world.maps["rosaria_map"].hexes[index][4]
            if world.maps["rosaria_map"].hexes[index][6] < 90:
                m.hexes[index][5] = world.maps["rosaria_map"].hexes[index][5]
            m.hexes[index][7] = world.maps["rosaria_map"].hexes[index][7]
            #m.hexes[index][10] = world.maps["rosaria_map"].hexes[index][10]
    ...
    def sg_patch22():
        ...
        for uid in castle.buildings.keys():
            ...
            if uid in all_buildings:
                castle.buildings[uid] = all_buildings[uid](uid)

                for d in _dict_value:
                    castle.buildings[uid].__dict__[d] = _dict_value[d]

    def sg_patch23():
        _dict_value = castle.buildings["sanctum"].__dict__.copy()
        castle.buildings["sanctum"] = DarkSanctum("sanctum")
        castle.buildings["sanctum"].__dict__ = _dict_value
