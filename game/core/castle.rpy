    ...
    class Castle(object):
        ...
        def _load_buildings(self):
            ...
            for uid in all_buildings:
                self.buildings[uid] = all_buildings[uid](uid)
        ...
        def end_week(self):
            ...
            if hasattr(all_actors["alexia"], "job_state") and all_actors["alexia"].job_state.job == "barmaid":
                avatar.gold += 5
                castle.treasury += 5
        ...
        def _update_buildings(self):
            ...
            for bld in self.buildings.values():
                ...
                if hasattr(bld, 'maintenance_discount'):
                    ...
                    if bld.name == "Castle Hall" and hasattr(all_actors["alexia"], "job_state") and all_actors["alexia"].job_state.job == "maid":
                        self.treasury -= int((bld.maintenance * 0.6) * (100 - self.surface_maintenance_reduction) / 10.0) / 10.0
                    else:
                        self.treasury -= int(bld.maintenance * (100 - self.surface_maintenance_reduction) / 10.0) / 10.0

                if hasattr(all_actors["alexia"], "job_state") and all_actors["alexia"].job_state.job == "research_assistant":
                    self.tech += bld.research + 2
                else:
        ...
        def _update_military_power(self):
            ...
            if self.buildings['pit'].capacity == self.buildings['pit'].driders:
                self.military += 250
        ...
        def _distribute_equipment(self):
            ...
            for bld in self.buildings.values():
                if hasattr(bld, 'equipment') and hasattr(bld, 'troops'):
                    unequipped = bld.troops - bld.equipment
                    if unequipped > 0:
                        bld.equipment += min(self._equipment,unequipped)
