init python:
    ...
    #Introduction
    event('orciad_too_early', triggers='map_res_105', conditions=("week < 4",), group='orciad_camp', priority=pr_map_res)
    #Triggered when Rowan enters the orc warband camp in Northern Rosaria for the first time.
    event('orciad_introduction', triggers='map_res_105', conditions=("week >= 4",), run_count=1, group='orciad_camp', priority=pr_map_res)
    #Camp Menu
...
label orciad_too_early:

scene bg26 with fade
show rowan neutral behind bg26
"Rowan saw an orc camp in the distance, teeming with orcs. More than he had seen in any one place since the war, probably best to avoid here for the time being."
ro "(I should wait to hear from the twins before visiting this place, otherwise it could go very poorly for me.)"
