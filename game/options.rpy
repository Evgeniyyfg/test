...
init -1 python hide:
    ...
    def test():
        if not hasattr(store, "avatar"):
            return

        if week == 0 and avatar.gold == 100 and store.old_gold == 0:
            return

        if store.old_gold != store.avatar.gold:
            renpy.show_screen("gold_animator", old_gold=store.old_gold)
            store.old_gold = store.avatar.gold
    ...
    config.interact_callbacks.append(test)
...
init python:
    ...
    build.directory_name = "seeds-of-chaos-0.2.64a"
    ...
    current_save_hover = 0

    if not persistent._save_name:
        persistent._save_name = {}

    def get_save_name(i):
        name = str(persistent._file_page) + "-" + str(i)

        return persistent._save_name.get(name, "")

    def put_save_name(line):
        name = str(persistent._file_page) + "-" + str(current_save_hover)
        persistent._save_name[name] = line