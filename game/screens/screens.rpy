...
screen file_picker():
    ...
    vbox:
        ...
        grid columns rows:
            ...
            for i in range(1, columns * rows + 1):
                button at file_eff:
                    hovered SetVariable("current_save_hover", i)
                    if not delete_mode:
                        action FileAction(i)
                    xfill True
                    ysize 165
                    ...
                    hbox:
                        ...
                        text FileSlotName(i, columns * rows) outlines []
                        if FileLoadable(name=i):
                            if rename_mode:
                                input default get_save_name(i) changed put_save_name:
                                    size 15
                                    color "#ffffff"
                                    kerning 1
                                    length 20
                            elif get_save_name(i):
                                text get_save_name(i):
                                    outlines []
                                    size 15
                                    color "#ffffff"
                            else:
                    ...
                    if FileLoadable(name=i) and delete_mode:
                        button:
                            xysize(35, 35)
                            text "X"
                            xalign 1.0
                            ...
                            action SetVariable("delete_mode", False), FileDelete(i)
                    vbox:
                        xalign 0.5
                        yalign 0.5
                        null height 8
    ...
    hbox:
        ...
        null width 10
        null width 10
        textbutton _("Delete Mode") at file_eff:
            xmaximum 100
            action ToggleVariable('delete_mode', True, False)

        null width 10
        null width 10
        textbutton _("Rename Mode") at file_eff:
            xmaximum 100
            action ToggleVariable('rename_mode', True, False)
...
screen gold_animator(old_gold):
    default display_value = old_gold
    default remainder = avatar.gold - old_gold
    default steps = 20
    default per_refresh = remainder / steps
    default final_show_for = 15
    ...
    timer 0.05 action If(steps == 0,
        true=If(final_show_for == 0, true=Hide("gold_animator"), false=[SetScreenVariable("display_value", avatar.gold), SetScreenVariable("final_show_for", final_show_for-1)]),
        false=[SetScreenVariable("display_value", display_value+per_refresh),SetScreenVariable("steps", steps-1)]) repeat True

    text "[remainder]" size 15 at move_up
    ...
    hbox:
        ...
        frame:
            ...
            add "gui/universal/Gold icon.png" xalign 0.78 yalign 0.6:
                zoom 0.5
            text "{color=" + colour + "}[display_value]{/color}" xalign 0.3 yalign 0.8

transform move_up():
    xalign 0.94
    yalign 0.2

    linear 0.5 yalign 0.06
